...
def poll_point(network, device, point, transport):
    if modbus_debug_poll: print('MODBUS: main looping function poll_point')
    connection = None
    if transport == "rtu":
    ...
    # debug
    if modbus_debug_poll: print("MODBUS:", {'network': network,
                            'device': device,
                            'transport': transport,
    ...
    try:
        ...
        if mod_point_type == ModbusPointType.readHoldingRegisters.name:
            ...
            array = read['array']
            # debug
            if modbus_debug_poll: print("MODBUS:",{'type': ModbusPointType.readHoldingRegisters.name, "val": val, 'array': array})
