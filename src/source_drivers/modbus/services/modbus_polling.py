...
from .modbus_registry import ModbusRegistryConnection
...
class ModbusPolling(EventServiceBase):
    ...
    def __poll_network_device(self, network: ModbusNetworkModel, device: ModbusDeviceModel):
        ...
        """
        current_connection: ModbusRegistryConnection = self.add_and_get_connection(network, device)
    ...
    def poll_point_not_existing(self, point: ModbusPointModel, device: ModbusDeviceModel, network: ModbusNetworkModel):
        ...
        connection: ModbusRegistryConnection = self.add_and_get_connection(network, device)
...
class RtuPolling(ModbusPolling):
    ...
    def add_and_get_connection(self, network: ModbusNetworkModel,
                               device: ModbusDeviceModel) -> ModbusRegistryConnection:
        registry: ModbusRtuRegistry = ModbusRtuRegistry()
        return registry.add_and_get_connection(network, device)
...
class TcpPolling(ModbusPolling):
    ...
    def add_and_get_connection(self, network: ModbusNetworkModel,
                               device: ModbusDeviceModel) -> ModbusRegistryConnection:
        registry: ModbusTcpRegistry = ModbusTcpRegistry()
        return registry.add_and_get_connection(network, device)
