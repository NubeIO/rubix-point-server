...
from bacpypes.errors import DecodingError, MissingRequiredParameter
...
class BACNetDriver(BIPSimpleApplication):
    ...
    def _init_bacnet(self):
        ...
        ldo = LocalDeviceObject(
            objectName="my name",
            objectIdentifier=int(22),
            maxApduLengthAccepted=int(1024),
            segmentationSupported="segmentedBoth",
            vendorIdentifier=int(22))

        # Send an empty support string
        print(ldo)
        pss = ServicesSupported()
        print(pss)
        # print(ldo.protocolServicesSupported)
        ...
        #         fcntl.ioctl(socket.socket(socket.AF_INET, socket.SOCK_DGRAM), 0x891b, struct.pack(b'256s', b'eth0'))[
        #         20:24])
        ...
        self.this_application = DiscoveryApplication(ldo, "192.168.0.100:47808")
    ...
    def _run_bacnet(self):
        ...
        while not self._done and self._parent_thread.is_alive():
            try:
                ...
                print("caught bacnet exception %s")
                continue
            else:
    ...
    #     except  Exception:
    #         return "error write property cast error %r"
    #
    ...
    def scan(self):
        addr = GlobalBroadcast()  # RemoteBroadcast $ LocalBroadcast
